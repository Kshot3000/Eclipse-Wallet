...
const signAsync = async (msg, privKey) => {
    ...
    // NOTE: concat explicitly — injected sha512 implementations may only accept a single buffer.
    const rBytes = await sha512a(concatB(e.prefix, m)); // r = SHA512(dom2(F, C) || prefix || PH(M))
...
const sign = (msg, privKey) => {
    ...
    // NOTE: concat explicitly — injected sha512 implementations may only accept a single buffer.
    const rBytes = sha512s(concatB(e.prefix, m)); // r = SHA512(dom2(F, C) || prefix || PH(M))
